    ...
    def mutate(self, genome):
        ...
        for y in range(height):
            for x in range(left, right):
                # Don't mutate the bottom row
                if y == height - 1:
                    continue

                if random.random() < 0.05: # 5% chance to mutate a tile
                    # Prevent pipes from floating by ensuring they are on the bottom row
                    ...
                    # If it's a pipe top or vertical pipe, ensure right side is also updated
...
class Individual_DE(object):
    ...
    def generate_children(self, other):
        ...
        if not self.genome or not other.genome:
            # Fallback: just return a copy of the parent
